...
## folder where alignments, logs, and error logs will go
baseDir='/home/albertf/mahlon/illumina/2020.07.31_FPFA001_TDH3pr_dsRed_TFT_sorts/'

## make sure we have the directories needed to run the alignment pipeline
for thisDir in {logs,error_logs,alignments}
do
    if [ ! -d "${baseDir}"/"${thisDir}" ]
then 
    mkdir -v "${baseDir}"/"${thisdir}"
else
    echo "${thisDir} exists"
    fi
done
...
mapScript=${baseDir}scripts/mapFilterCount_4Cluster.sh
